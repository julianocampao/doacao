...
public class Mailer {
	...
	public void notificarInstituicaoItem(Instituicao instituicao, Item item) {
		...
		mensagem.setText("O item " + item.getDescricao() + " está aguardando alguma instituição interassada no portal doarte");
	...
	public void notificarUsuario(Instituicao instituicao, Item item) {
		SimpleMailMessage mensagem = new SimpleMailMessage();
		mensagem.setFrom("doarte@doartetcc.com");
		...
		mensagem.setText(
				"A instituição interessada nome " + instituicao.getNome() + " contato " + instituicao.getContato()
						+ " e email " + instituicao.getEmail() + "aguarda contato e combinar a entrega! Obrigado pela sua doação!");
